...
export function createSocketServer(server: http.Server) {
  ...
  io.on('connection', (socket) => {
    ...
    // Community channel: join a channel room
    socket.on('community:join', (channelId: string) => {
      socket.join(`channel:${channelId}`);
      console.log(`User joined channel room: channel:${channelId}`);
    });

    // Community channel: leave a channel room
    socket.on('community:leave', (channelId: string) => {
      socket.leave(`channel:${channelId}`);
      console.log(`User left channel room: channel:${channelId}`);
    });

    // Community: typing indicator
    socket.on('community:typing', (data: { channelId: string; isTyping: boolean }) => {
      const user = (socket as any).user;
      if (user) {
        socket.to(`channel:${data.channelId}`).emit('community:typing', {
